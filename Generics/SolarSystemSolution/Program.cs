...
Node<SolarItem> mondNode = new(new SolarItem("Mond", SolarItemType.Trabant));
earthNode.AddChild(mondNode);
mondNode.AddChild(new SolarItem("Test", SolarItemType.Trabant));
...
void DisplaySolarSystemDepth(Node<SolarItem> solarNode, int depth = 0)
{
    if (depth == 0)
